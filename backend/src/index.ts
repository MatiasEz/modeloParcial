...
import comprarRoutes from './routes/comprarRoutes';
...
declare module 'express-session' {
  export interface SessionData {
	...
	// carrito: []| any;//en user guardaremos datos de interes
	...
	config():void{
        ...
        //Variables globales
		 this.app.use((req,res,next)=>{
			...
			this.app.locals.msg_session=req.flash('msg_session');
			// ACA VAN VARIABLES GLOBALES:
			next();
		});
	}
    
	// Aca van las rutas de la app:
	// Aca van las rutas de la app:
    // Aca van las rutas de la app:
	routes():void{
		...
		this.app.use("/comprar",comprarRoutes); 
